...
Node* oldMergeSort(Node* list, int n) {
	if (n < 2) return list;
...
Node* mergeSort(Node* list, int n) {
	...
	while (current != nullptr) {
		...
		for (i = 0; i < 32 && nodeArr[i] != nullptr; i++) {
			current = mergeList(current, nodeArr[i]);
			nodeArr[i] = nullptr;
		}
		if (i == 32) i = 31;
	...
	for (i = 0; i < 32; i++) {
		current = mergeList(current, nodeArr[0]);
	}
	return current;
}
